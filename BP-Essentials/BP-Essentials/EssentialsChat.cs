    ...
    public class EssentialsChatPlugin : SvPlayer {
        ...
        public static bool SvGlobalChatMessage(SvPlayer player, ref string message)
        {
            try
            {
                ...
                {
                    player.SendToAllOthers(Channel.Unsequenced, 10, $"<color=#f45342>[STAFF]</color> <color=#f47141>{player.playerData.username}: </color><color=#35dfe8>{message}</color>");
                    return true;
                }
                if (!ChatBlock && !LanguageBlock)
                    return false;
                string filteredmsg = Chat.LangAndChatBlock.Run(player, message);
                player.SendToAllOthers(Channel.Unsequenced, 10, $"{player.playerData.username}: {filteredmsg}");
                return true;
            }
