    ...
    class Free : EssentialsChatPlugin
    {
        public static void Run(SvPlayer player, string message)
        {
            ...
            if (!string.IsNullOrEmpty(arg1))
            {
                ...
                player.Send(SvSendType.Self, Channel.Unsequenced, ClPacket.GameMessage, $"<color={infoColor}>Freed</color> <color={argColor}>" + shPlayer.username + $"</color><color={infoColor}>.</color>");
            }
            else
