...
namespace BPEssentials.ExtensionMethods
{
    public static class ExtensionPlayerItem
    {
        ...
        {
            player.SavePosition(player.Client.Position, player.Client.Rotation, player.Client.GetPlaceIndex());
        }
        ...
        {
            player.SavePosition();
            player.Client.svPlayer.SvRestore(targetPlayer.Position, targetPlayer.Rotation, targetPlayer.GetPlaceIndex());
        }
    }
}
